...
function SkeletonRow() {
  return (
    <tr className="border-b border-gray-50 dark:border-gray-800/50 animate-pulse">
      ...
      <td className="px-6 py-4">
        <div className="ml-auto h-6 w-16 rounded-lg bg-gray-100 dark:bg-gray-800" />
      </td>
    </tr>
  );
}

function SubCategoryModal({ subCategory, onClose, action }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const result = isEdit
        ? await action(subCategory._id, {
            name: name.trim(),
          })
        : await action({ name: name.trim() });
      onClose();
    } catch (err) {
...
function DeleteModal({ subCategory, onClose, action }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-xl">
        ...
        <p className="mt-1 text-base text-[#00000080] dark:text-gray-500">
          This will permanently remove this subcategory. Posts and users in this
        ...
        <div className="mt-5 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={deleting}
            className="rounded-xl bg-[#FF0000] px-5 py-2.5 text-xl font-medium text-white hover:bg-red-600 transition disabled:opacity-60 disabled:cursor-not-allowed"
          >
...
export default function SubCategoriesPage() {
  ...
  async function handleDrop(e, targetId) {
    ...
    setDragOverId(null);
    try {
      await reorderSubCategory(updated.map((c) => c._id));
    } catch {
      fetchSubCategories();
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400 dark:text-gray-500">
            Manage sub-categories used across the platform.
          </p> */}
        </div>
        <button
      ...
      {/* <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-[20px] flex flex-col gap-17  bg-[#2F3134] p-5">
